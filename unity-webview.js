var unityWebView =
{
    ...
    init : function (name) {
        ...
        if ($containers.length === 0) {
            $('<div style="position: absolute; left: 0px; width: 100%; height: 100%; top: 0px;pointer-events: none;"><div class="webviewContainer" style="overflow: hidden; position: relative; width: 100%; height: 100%; z-index: 1;"></div></div>')
                .appendTo($('.webgl-content'));
        }
        var $last = $('.webviewContainer:last');
        var clonedTop = parseInt($last.css('top')) - 100;
        var $clone = $last.clone().insertAfter($last).css('top', clonedTop + '%').css("z-index", "-10");
        var $iframe =
            $('<iframe style="width: 85% !important;margin: 0 auto;display: block;top: 28% !important;position: relative;border: none;aspect-ratio: 1136/405;pointer-events: all;"></iframe>')
    ...
    setMargins: function (name, left, top, right, bottom) {
        var container = $('#contents_main');
        var w0 = container.width();
