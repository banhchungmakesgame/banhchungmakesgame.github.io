...
var ASM_CONSTS = {
  6908588: function() {Module['emscripten_get_now_backup'] = performance.now;},  
 6908643: function($0) {performance.now = function() { return $0; };},  
 6908691: function($0) {performance.now = function() { return $0; };},  
 6908739: function() {performance.now = Module['emscripten_get_now_backup'];},  
 6908794: function() {return Module.webglContextAttributes.premultipliedAlpha;},  
 6908855: function() {return Module.webglContextAttributes.preserveDrawingBuffer;},  
 6908919: function() {return Module.webglContextAttributes.powerPreference;}
};
  ...
  function _AVPPlayerInsertVideoElement(path, idValues, externalLibrary)
  	{
  		...
  		var vidObject = window.videoPool.get();
  		var vid = vidObject.video;
  		console.log('use video from pool');
  		...
  		else if (externalLibrary == 3)
  		{
  			...
  			console.log('overrideNative: ' + overrideNative);
  			...
  			{
  				vid.type = 'application/vnd.apple.mpegurl';
  				fetch(path)
  		...
  		vid.onloadedmetadata = function()
  		{
  			if(_is_iOS())
  			{
  				console.log('vid loadeddata true');
  				vid.play().then(() => {
  					vid.muted = false;
  				});
  			}
  ...
  function _JSGetFileByKey(fileUrl) {
  		...
  		var file = UTF8ToString(fileUrl);
  
  		try {
  			var transaction = window.streamDB.transaction(window.storeName, (IDBTransaction.READ_ONLY ? IDBTransaction.READ_ONLY : 'readonly'));
  		}
  		catch (ex) {
  			console.log(ex);
  			return;
  		...
  		try {
  			var objectStore = transaction.objectStore(window.storeName);
  			...
  			request.onsuccess = function(event) {
  				var blobUrl = null;
  				if(request.result)
  					blobUrl = URL.createObjectURL(request.result.File);
  				var getFileJson = { fileUrl: file, returnUrl: blobUrl }
  				SendMessage('IndexedDBService', 'OnGetFileSuccess', JSON.stringify(getFileJson));
  			}
  
  			request.onerror = function(event) {
  				var getFileJson = { fileUrl: file, returnUrl: null }
  				SendMessage('IndexedDBService', 'OnGetFileSuccess', JSON.stringify(getFileJson));
  			}			
  		}
  		catch (ex) {
  ...
  function _JSRequestIndexedDB(dbName, storeName, dbVersion) {
          
  		window.dbName = UTF8ToString(dbName);
  		window.storeName = UTF8ToString(storeName);
  		
  		var request = indexedDB.open(window.dbName, dbVersion);
  
  		request.onerror = function (event) {
  			SendMessage('IndexedDBService', 'OnRequestDBError', 'error');
  		};
  
  		request.onsuccess = function (event) {
  			console.log("Success creating/accessing IndexedDB database");
  			window.streamDB = event.target.result;
  			window.streamDB.onerror = function (event) {
  				console.log("Error creating/accessing IndexedDB database");
  			};
  			SendMessage('IndexedDBService', 'OnRequestDBSuccess');
  		}
  		
  		request.onupgradeneeded = function (event) {
  			window.streamDB = event.target.result;
  			try {
  				window.streamDB.createObjectStore(window.storeName, {
  					keyPath: "KeyURL",
  					autoIncrement: false
  				});
  			} catch (ex) {
  			...
  			SendMessage('IndexedDBService', 'OnDBUpgraded');
  		};
  	}

  function _JSSaveFileToDB(fileUrl, byteArray, arrayLength) {
  		...
  		try {
  			var transaction = window.streamDB.transaction(window.storeName, (IDBTransaction.READ_WRITE ? IDBTransaction.READ_WRITE : 'readwrite'));
  		}
  		catch (ex) {
  			console.log(ex);
  			return;
  		...
  		try {
  			var objectStore = transaction.objectStore(window.storeName);
  			const newArray = new ArrayBuffer(arrayLength);
  			const newByteArray = new Uint8Array(newArray);
  			for(var i = 0; i < arrayLength; i++)
  				newByteArray[i] = HEAPU8[byteArray + i]; 
  			
  			var blob = new Blob([newByteArray]);
  			var fileObj = {
  				KeyURL: file,
  				File: blob
  			}
  
  			var putRequest = objectStore.put(fileObj);
  			var URL = window.URL || window.webkitURL;
  			putRequest.onsuccess = function() {
  				var getRequest = objectStore.get(putRequest.result);
  				getRequest.onsuccess = (event) => {
  					var addedFileJson = { fileUrl: file, returnUrl: URL.createObjectURL(getRequest.result.File) }
  					SendMessage('IndexedDBService', 'OnSaveFileSuccess', JSON.stringify(addedFileJson));
  				};
  			}    
  		}
  		catch (ex) {
  ...
  function _JSUpdateKeyURL(fileUrl, byteArray, arrayLength) {
  		...
  		try {
  			...
  			var blob = new Blob([newByteArray], {type: 'application/x-mpegURL'});
  			var fileObj = {
  				KeyURL: file,
  				File: blob
  			}
  			...
  			putRequest.onsuccess = function() {
  				...
  				getRequest.onsuccess = (event) => {
  					var addedFileJson = { fileUrl: file, returnUrl: URL.createObjectURL(getRequest.result.File) }
  					SendMessage('IndexedDBService', 'OnUpdateFileSuccess', JSON.stringify(addedFileJson));
  				};
  			}
  		}
  		catch (ex) {
...
var asmLibraryArg = {
  ...
  "JSGetFileByKey": _JSGetFileByKey,
  "JSRequestIndexedDB": _JSRequestIndexedDB,
  "JSSaveFileToDB": _JSSaveFileToDB,
  "JSUpdateKeyURL": _JSUpdateKeyURL,
...
unexportedRuntimeFunction('es', false);
unexportedRuntimeFunction('es__user', false);
